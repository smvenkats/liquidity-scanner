...
Before writing a script, check `execution/` per directive. Only create new scripts if none exist.
...
- Example: you hit an API rate limit → you look into the API → find a batch endpoint that would fix it → rewrite script to accommodate → test → update directive.
